...
- Edit `<proj_root>/bheemboy/settings.py` and give a value to `SECRET_KEY`, add auth details to `DATABASES`, set `DEBUG` to False, SMTP server in `EMAIL_HOST`, and add the hostname (such as `coursereg.iisc.ac.in`) to `ALLOWED_HOSTS`.
