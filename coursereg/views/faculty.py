...
def faculty(request):
    ...
    try:
        ...
        messages.error(request, 'You listed as faculty in the portal. Please avoid attempts to use  the system it in unauthorized ways. This attempt has been logged.')
...
def student_details(request):
    ...
    try:
        ...
        messages.error(request, 'You are not the adviser of the requested student. Please avoid attempts to use  the system it in unauthorized ways. This attempt has been logged.')
        log = logging.getLogger(__name__)
...
def participant_advisor_act(request):
    ...
    try:
        ...
        messages.error(request, 'You are not the adviser of the requested participant student. Please avoid attempts to use the system it in unauthorized ways. This attempt has been logged.')
        log = logging.getLogger(__name__)
        log.warn("\n**************************************\nSUSPICIOUS_ACTIVITY::ATTEMPT TO MASQUERADE AN ADVISER by:"+str(request.user)+"\n"+str(request.META)+'\n\n')
        return redirect('coursereg:index')
